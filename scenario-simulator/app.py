...
st.markdown("Track risk-informed scenario shifts across APEC economies to support adaptive program management.")

# Load risk signals
try:
    signal_df = pd.read_csv("data/risk_signals.csv")
    signal_df.columns = signal_df.columns.str.strip()  # Clean column headers
...
try:
    matrix_df = pd.read_csv("scenario-simulator/data/full_apec_rise_scenario_matrix.csv")
...
with st.sidebar:
    ...
    # Assumption filter (from signal data)
    ...
    # Scenario filter (force all 3 options to appear)
...
# Display results
st.subheader("📋 Filtered Scenario Signals")
if filtered.empty:
    st.info("No matching signals found.")
else:
    st.dataframe(filtered, use_container_width=True)

# Show scenario matrix below (optional)
if not matrix_df.empty:
    st.subheader("🧭 Scenario Matrix Reference")
